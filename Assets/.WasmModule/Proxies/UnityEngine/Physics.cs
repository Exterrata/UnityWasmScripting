...
public static class Physics {
	...
	public static bool Raycast(Vector3 origin, Vector3 direction, out RaycastHit hitInfo, float maxDistance = Mathf.Infinity, int layerMask = DefaultRaycastLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal) => internal_Raycast(origin, direction, out hitInfo, maxDistance, layerMask, queryTriggerInteraction);

	public static int SphereCastNonAlloc(Vector3 origin, float radius, Vector3 direction, RaycastHit[] results, float maxDistance = Mathf.Infinity, int layerMask = DefaultRaycastLayers, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal) => internal_SphereCastNonAlloc(origin, radius, direction, results, maxDistance, layerMask, queryTriggerInteraction);
	...
	private static unsafe int internal_SphereCastNonAlloc(Vector3 origin, float radius, Vector3 direction, RaycastHit[] results, float maxDistance, int layerMask, QueryTriggerInteraction queryTriggerInteraction) {
		fixed (RaycastHit* resultsPtr = results) {
			return Physics_func_SphereCastNonAlloc_vector3_float_vector3_raycasthit_int_float_int_querytriggerinteraction((long)Unsafe.AsPointer(ref origin), radius, (long)Unsafe.AsPointer(ref direction), (long)resultsPtr, results.Length, maxDistance, layerMask, (int)queryTriggerInteraction);
		}
	}
	...
	private static unsafe bool internal_Raycast(Vector3 origin, Vector3 direction, out RaycastHit hitInfo, float maxDistance, int layerMask, QueryTriggerInteraction queryTriggerInteraction) {
		hitInfo = default;
		int result = Physics_func_Raycast_vector3_vector3_raycasthit_float_int_querytriggerinteraction((long)Unsafe.AsPointer(ref origin), (long)Unsafe.AsPointer(ref direction), (long)Unsafe.AsPointer(ref hitInfo), maxDistance, layerMask, (int)queryTriggerInteraction);
