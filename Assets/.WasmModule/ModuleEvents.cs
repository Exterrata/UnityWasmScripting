...
public static partial class Module {
	...
	public static void CallUpdate() {
		foreach (MonoBehaviour behaviour in UpdateSortedBehaviours.Values) {
			if (Callbacks[behaviour.GetType()].TryGetValue(ScriptEvent.Update, out MethodInfo method))
	...
	public static void CallLateUpdate() {
		foreach (MonoBehaviour behaviour in UpdateSortedBehaviours.Values) {
			if (Callbacks[behaviour.GetType()].TryGetValue(ScriptEvent.LateUpdate, out MethodInfo method))
				method.Invoke(behaviour, null);
		}
	}
	...
	public static void CallFixedUpdate() {
		foreach (MonoBehaviour behaviour in UpdateSortedBehaviours.Values) {
			if (Callbacks[behaviour.GetType()].TryGetValue(ScriptEvent.FixedUpdate, out MethodInfo method))
				method.Invoke(behaviour, null);
		}
	}
