...
namespace WasmScripting {
	public abstract class WasmBinding {
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		protected static T IdToClass<T>(StoreData data, long id) where T : class => (T)data.AccessManager.ToWrapped(id).Target;
		
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		protected static ref T IdToStruct<T>(StoreData data, long id) where T : struct => ref Unsafe.Unbox<T>(data.AccessManager.ToWrapped(id).Target);
