    ...
    public static class WasiStubs
    {
        public static void DefineWasiFunctions(Linker linker)
        {
            ...
            linker.DefineFunction(
                ...
                "environ_sizes_get",
                (int environcPtr, int environsPtr) => 0
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_prestat_get",
                (int fd, int prestatPtr) => 8
            );
            linker.DefineFunction("wasi_snapshot_preview1", "fd_close", (int _) => 8);
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_advise",
                (int _, long _, long _, int _) => 8
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_read",
                (int _, int _, int _, int _) => 8
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_seek",
                (int _, long _, int _, int _) => 8
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_write",
            ...
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_pread",
                (int _, int _, int _, long _, int _) => 8
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "fd_readdir",
            ...
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "path_filestat_get",
                (int _, int _, int _, int _, int _) => 0
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "path_readlink",
                (int _, int _, int _, int _, int _, int _) => 0
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "path_unlink_file",
            ...
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "path_open",
                (int _, int _, int _, int _, int _, long _, long _, int _, int _) => 0
            );
            linker.DefineFunction(
                "wasi_snapshot_preview1",
                "poll_oneoff",
                (int _, int _, int _, int _) => 0
            );
