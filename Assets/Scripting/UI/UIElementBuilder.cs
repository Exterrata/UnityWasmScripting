...
namespace UI {
	...
	public static partial class UIElementBuilder {
		...
		public static T Element<T>(params UIOperation[] operations) where T : VisualElement, new() {
			T element = new();
			ApplyOperation(element, operations);
			return element;
		}
		
		public static T Element<T>(out T element, params UIOperation[] operations) where T : VisualElement, new() {
			element = new();
			ApplyOperation(element, operations);
			return element;
		}
		...
		public static Subscribe Subscribe<TEvent>(EventCallback<TEvent> callback, TrickleDown trickleDown = TrickleDown.NoTrickleDown) where TEvent : EventBase<TEvent>, new()
			=> new() { Callback = new UICallback<TEvent>(callback, trickleDown) };
