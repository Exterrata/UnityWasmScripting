...
namespace WasmScripting {
	public partial class WasmBindingGenerator : EditorWindow {
		...
		private void BuildUI() {
			rootVisualElement.styleSheets.Add(StyleSheetBuilder.Build(b => b.Add(".tab-list TextElement:checked", s => s.BorderTopLeftRadius(3).BorderTopRightRadius(3).BackgroundColor(new(0.345f, 0.345f, 0.345f))).Add(".section-box", s => s.Margin(2, 4).BorderTopLeftRadius(5).BorderTopRightRadius(5).BorderBottomLeftRadius(5).BorderBottomRightRadius(5).BackgroundColor(new(0.19f, 0.19f, 0.19f))).Add(".unity-base-field__label", s => s.MinWidth(100).FlexShrink(1))));
				...
				Element(
					...
					Element( // left side type list
						...
						Element( // type list header
							...
							Element<ToolbarSearchField>(Subscribe<ChangeEvent<string>>(SearchTypeList), MarginLeft(0), MarginRight(0), FlexShrink(1), Width(new Length(100, LengthUnit.Percent)))
						),
						Element( // type list
							...
							PaddingLeft(2),
							PaddingRight(2),
							Element(FlexGrow(1), new TextListElement<Type>(SetTypeItemColor, SelectTypeListItem))
						)
					),
					Element( // center member list
						...
						Element( // member list header
							...
							Element(FlexGrow(1)),
							Element(out _memberListSearchElement, Subscribe<ChangeEvent<string>>(SearchMemberList), MarginLeft(0), MarginRight(0), FlexShrink(1), Width(150))
						),
						Element( // member list
							...
							PaddingLeft(2),
							PaddingRight(2),
							Element(FlexGrow(1), new TextListElement<string>(SetMemberItemColor, SelectMemberListItem) { MultiSelect = true })
						)
					),
					Element( // right side binding settings
						...
						Element(out VisualElement bindingOverrideIdField, AddClass("section-box"), MarginTop(4), PaddingTop(2), PaddingBottom(2), new TextField("Override ID:") { style = { flexGrow = 1 } }),
						Element(
