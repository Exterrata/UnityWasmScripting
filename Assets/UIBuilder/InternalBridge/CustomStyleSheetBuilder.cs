    ...
    public class CustomStyleSheetBuilder
    {
        ...
        public void Add(string selector, IStyle style)
        {
            ...
            foreach (PropertyInfo prop in StyleProperties)
            {
                ...
                if ((StyleKeyword)keyword! != StyleKeyword.Undefined)
                    continue;
        ...
        private void ProcessStyleProperty(object styleValue)
        {
            switch (styleValue.GetType().GetProperty("value")?.GetValue(styleValue))
            {
                ...
                case Rotate rotate:
                    _builder.AddValue(
                        new Dimension(rotate.angle.value, ConvertUnit(rotate.angle.unit))
                    );
                    break;
                case Scale scale:
                ...
                case Translate translate:
                    _builder.AddValue(
                        new Dimension(translate.x.value, ConvertUnit(translate.x.unit))
                    );
                    _builder.AddValue(
                        new Dimension(translate.y.value, ConvertUnit(translate.y.unit))
                    );
                    _builder.AddValue(translate.z);
                    break;
                case TransformOrigin transformOrigin:
                    _builder.AddValue(
                        new Dimension(transformOrigin.x.value, ConvertUnit(transformOrigin.x.unit))
                    );
                    _builder.AddValue(
                        new Dimension(transformOrigin.y.value, ConvertUnit(transformOrigin.y.unit))
                    );
                    _builder.AddValue(transformOrigin.z);
                    break;
                case Background background:
                    if (background.vectorImage != null)
                        _builder.AddValue(background.vectorImage);
                    if (background.sprite != null)
                        _builder.AddValue(background.sprite);
                    if (background.renderTexture != null)
                        _builder.AddValue(background.renderTexture);
                    if (background.texture != null)
                        _builder.AddValue(background.texture);
                    break;
                ...
                case Cursor cursor:
                    if (cursor.texture != null)
                        _builder.AddValue(cursor.texture);
        ...
        private List<(StyleSelectorPart[], StyleSelectorRelationship)> ParseSelector(
            string selector
        )
        {
            ...
            while (index < selector.Length)
            {
                ...
                while (
                    ...
                    && selector[index] != ':'
                    && selector[index] != '>'
                    && !char.IsWhiteSpace(selector[index])
                )
                {
                    index++;
                }
