...
namespace Koneko.UIBuilder.InternalBridge
{
    public static class MeshGenerationContextExtras
    {
        ...
        public static void DrawText(
            this MeshGenerationContext ctx,
            string text,
            Vector2 position,
            int fontSize,
            Color color
        )
        {
            MeshGenerationContextUtils.TextParams textParams =
                MeshGenerationContextUtils.TextParams.MakeStyleBased(ctx.visualElement, text);
        ...
        {
            ctx.Rectangle(
                MeshGenerationContextUtils.RectangleParams.MakeSolid(
                    rect,
                    color,
                    ContextType.Editor
                )
